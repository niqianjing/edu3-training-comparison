...
from gym.envs.hi_cl_12.hi_cl12_controller_config import Hicl12ControllerCfg
from gym.utils.math import *
from gym.envs import LeggedRobot
from isaacgym import gymapi, gymutil
import numpy as np
from typing import Tuple, Dict
from .hi_cl12_utils import (
    FootStepGeometry,
    SimpleLineGeometry,
    VelCommandGeometry,
    smart_sort,
...
class Hicl12Controller(LeggedRobot):
    ...
    def _reward_contact_force(self):
        # Reward contact force on feet
        contact_force = self.contact_forces[:, self.feet_ids, 2]
        return -torch.sum(contact_force, dim=1)
    

    def _reward_contact_orin(self):
        # Reward contact force on feet
        contact_0 = self.contact_forces[:, self.feet_ids[0], 2] > 50.0
        contact_1 = self.contact_forces[:, self.feet_ids[1], 2] > 50.0
        ...
        rew = torch.exp(
            -(torch.norm(feet_eular_0*contact_0, dim=1) + torch.norm(feet_eular_1*contact_1, dim=1)) * 100
        )
        return rew
    

    def _reward_contact_feet_slip(self):
