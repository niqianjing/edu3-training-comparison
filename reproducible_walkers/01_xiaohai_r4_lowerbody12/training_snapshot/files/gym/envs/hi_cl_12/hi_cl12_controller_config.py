...
class Hicl12ControllerCfg(LeggedRobotCfg):
    ...
    class commands(LeggedRobotCfg.commands):
        ...
        class ranges(LeggedRobotCfg.commands.ranges):
            ...
            dstep_width = [0.22, 0.25]  # [0.2, 0.4] # min max [m]

            lin_vel_x = [-2.5, 2.5]  # [-3.0, 3.0] # min max [m/s]
            lin_vel_y = 1.  # 1.5   # min max [m/s]
            yaw_rate = 0.0  # min max [rad/s]
...
class Hicl12ControllerRunnerCfg(LeggedRobotRunnerCfg):
    do_wandb = True
    seed = -1
