"""Deterministic 5 ms / 20 ms position-target pulse in Xiaohai Isaac Gym."""
import json
...
def main():
    ...
    kp, kd, limits = env.p_gains.clone(), env.d_gains.clone(), env.torque_limits.clone()
    ...
    for idx, name in enumerate(names):
        ...
        for step in range(20):
            ...
            if step + 1 in (5, 20):
                samples[str(step + 1)] = {"q_delta": float((env.dof_pos[0, idx] - q0[0, idx]).item()), "velocity": float(env.dof_vel[0, idx].item()), "torque": float(tau[0, idx].item())}
        results[name] = samples
    body_props = env.gym.get_actor_rigid_body_properties(env.envs[0], env.actor_handles[0])
    payload = {"engine": "isaac", "route": "lower", "dt": float(params.dt), "joint_names": names, "total_mass_kg": sum(float(prop.mass) for prop in body_props), "results": results}
    out = "/home/zero/xiaohai_training/lowerbody_isaac_pulse.json"
