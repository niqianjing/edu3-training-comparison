...
def main() -> None:
    ...
    ap.add_argument("--name", default="Xiaohai_model15000_PORTABLE_DCMOTOR_MuJoCo")
        ...
        for step in range(total_steps + 1):
            ...
            if step % control_stride == 0:
                ...
                if step == 0:
                    ...
                    history[:-1] = history[1:]
                    history[-1] = obs
                policy_obs_control.append(history.reshape(-1).copy())
                with torch.inference_mode():
                    action_lab = policy(torch.from_numpy(history.reshape(1, -1))).cpu().numpy()[0]
                action_mj = action_lab[lab_to_mj]
                reference_now_mj = reference_mj(phase)
                raw_target = reference_now_mj + action_mj * 0.25
                raw_target_oob.append(bool(np.any(limited & ((raw_target < joint_ranges[:, 0]) | (raw_target > joint_ranges[:, 1])))))
                if args.target_clamp:
                    # Match Isaac Lab soft_joint_pos_limit_factor=0.95: keep 95%
                    # of the full centered range, so trim 2.5% at each end.
                    margin = 0.025 * (joint_ranges[:, 1] - joint_ranges[:, 0])
                ...
                # Feed the post-clamp action back into history, matching Isaac env.step.
                effective_action_mj = (target_mj - reference_now_mj) / 0.25
                effective_action_lab = np.empty(23, dtype=np.float32)
                for mj_i, lab_i in enumerate(lab_to_mj):
                    effective_action_lab[lab_i] = effective_action_mj[mj_i]
                last_action_lab = effective_action_lab
                actual_target_oob.append(bool(np.any(limited & ((target_mj < joint_ranges[:, 0]) | (target_mj > joint_ranges[:, 1])))))

            computed_tau = 80.0 * (target_mj - data.qpos[qadr]) - 1.0 * data.qvel[dadr]
            curve_vel = np.clip(data.qvel[dadr], -velocity_at_effort_limit, velocity_at_effort_limit)
            torque_speed_top = saturation * (1.0 - curve_vel / velocity_limit)
            torque_speed_bottom = saturation * (-1.0 - curve_vel / velocity_limit)
            max_effort = np.minimum(torque_speed_top, effort)
            min_effort = np.maximum(torque_speed_bottom, -effort)
            tau = np.clip(computed_tau, min_effort, max_effort)
            data.ctrl[:] = tau
    ...
    metrics = {
        ...
        "actuator_model": "explicit_pd_plus_dc_torque_speed_curve",
    ...
    np.savez_compressed(
        ...
        policy_obs_control=np.asarray(policy_obs_control),
    )
