...
def main():
    ...
    for prim in stage.Traverse():
        ...
        if "/Robot/" not in path and "/ground" not in path.lower():
            continue
        ...
        for attr in prim.GetAttributes():
            name = attr.GetName()
            if "friction" in name.lower() or "restitution" in name.lower():
                val = attr.Get()
                if val is not None:
                    attrs[name] = str(val)
        if attrs:
            material_attrs.append({"path": path, "attrs": attrs})
    payload = {"collision_prims": collision_prims, "material_attrs": material_attrs}
    with open(args_cli.out, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)
    print("COLLISION_MATERIAL_PROBE=" + json.dumps({"collision_count": len(collision_prims), "material_count": len(material_attrs)}), flush=True)
    return
