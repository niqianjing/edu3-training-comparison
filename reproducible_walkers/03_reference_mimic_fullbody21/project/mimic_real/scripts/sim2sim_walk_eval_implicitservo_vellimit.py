...
class MotionLoader:
    ...
    def __init__(self, path, simulator_joint_names, device="cpu", add_static_frame=False):
        ...
        # Match Isaac: frame-zero linear/joint velocities are zero, while
        # root angular velocity uses the first forward quaternion difference.
        self.root_omega = torch.zeros((self.frame_num, 3), dtype=torch.float32, device=device)
        if self.frame_num > 1:
            q0 = self.root_rot[0].cpu().numpy()
            q1 = self.root_rot[1].cpu().numpy()
            r0 = R.from_quat(q0[[1, 2, 3, 0]])
            r1 = R.from_quat(q1[[1, 2, 3, 0]])
            omega0 = (r1 * r0.inv()).as_rotvec() * self.fps
            self.root_omega[0] = torch.tensor(omega0, dtype=torch.float32, device=device)
...
def main():
    ...
    # Isolated portability test: clamp targets to compiled joint ranges with
    # the same 5 percent soft-limit margin as the Isaac asset.
    joint_ids = np.asarray([mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, name) for name in MJ_NAMES])
    hard_ranges = model.jnt_range[joint_ids].astype(np.float64)
    centers = 0.5 * (hard_ranges[:, 0] + hard_ranges[:, 1])
    half_spans = 0.5 * (hard_ranges[:, 1] - hard_ranges[:, 0]) * 0.95
    target_lower = centers - half_spans
    target_upper = centers + half_spans

    # Rebuild the actuators as MuJoCo position servos and let the implicit
    # integrator solve the PD drive together with the rigid-body dynamics.
    # This is much closer to PhysX ImplicitActuator than precomputing torque
    # in Python and applying it one explicit step later.
    ...
    model.actuator_ctrlrange[:, 0] = target_lower
    model.actuator_ctrlrange[:, 1] = target_upper
    model.actuator_forcelimited[:] = 1
    model.actuator_forcerange[:, 0] = -TAU_LIMIT
    model.actuator_forcerange[:, 1] = TAU_LIMIT
    # Isaac CircularBuffer repeats the first appended observation through all
    # ten history slots; zero-filling them changes the model input contract.
    q0 = data.qpos[-N:].astype(np.float64)
    dq0 = data.qvel[-N:].astype(np.float64)
    q0_lab = q0[np.asarray(lab_to_mj)]
    dq0_lab = dq0[np.asarray(lab_to_mj)]
    quat0 = data.sensor("orientation").data[[1, 2, 3, 0]].astype(np.float64)
    omega0 = data.sensor("angular-velocity").data.astype(np.float64)
    gravity0 = qinv_apply(quat0, np.asarray([0.0, 0.0, -1.0]))
    obs0 = np.zeros((1, SINGLE_OBS), dtype=np.float32)
    obs0[0, 0:3] = omega0
    obs0[0, 3:6] = gravity0
    obs0[0, 6:29] = q0_lab
    obs0[0, 29:52] = dq0_lab
    obs0[0, 52:75] = 0.0
    obs0[0, 75:81] = [0.0, 1.0, 0.0, 1.0, 0.0, 1.0]
    hist = np.tile(obs0, (1, HISTORY)).astype(np.float32)
    ...
    try:
        for count in range(total_steps):
            if count % decimation == 0:
                ...
                raw_target_mj = ref_mj + delta_mj
                target_mj = np.clip(raw_target_mj, target_lower, target_upper)
            ...
            q_before = data.qpos[-N:].copy()

            mujoco.mj_step(model, data)

            dq_limited = np.clip(data.qvel[-N:], -6.28, 6.28)

            data.qvel[-N:] = dq_limited

            data.qpos[-N:] = q_before + dq_limited * model.opt.timestep

            mujoco.mj_forward(model, data)
            if count % decimation == 0:
                taus.append(data.actuator_force.copy())
