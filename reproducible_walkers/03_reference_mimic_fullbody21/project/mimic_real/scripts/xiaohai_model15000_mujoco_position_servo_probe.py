...
def main() -> None:
    ...
    ap.add_argument(
        "--actuator-mode",
        choices=("explicit-pd", "position"),
        default="explicit-pd",
        help="explicit clipped torque PD, or MuJoCo affine position servo",
    )
    ap.add_argument("--velocity-clamp", type=float, default=None)
    ap.add_argument("--sliding-friction", type=float, default=None)
    args = ap.parse_args()
    ...
    if args.sliding_friction is not None:
        model.geom_friction[:, 0] = args.sliding_friction
    ...
    if args.actuator_mode == "position":
        # MuJoCo affine position servo: force is limited in Nm, ctrl is angle.
    ...
    try:
        for step in range(total_steps + 1):
            ...
            if step < total_steps:
                ...
                if args.velocity_clamp is not None:
                    data.qvel[dadr] = np.clip(
                        data.qvel[dadr], -args.velocity_clamp, args.velocity_clamp
                    )
                    mujoco.mj_forward(model, data)
    ...
    metrics = {
        ...
        "velocity_clamp_rad_s": args.velocity_clamp,
