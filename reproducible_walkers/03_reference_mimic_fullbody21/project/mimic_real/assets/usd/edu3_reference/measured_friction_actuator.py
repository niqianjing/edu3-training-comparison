...
class MeasuredFrictionDelayedPDActuator(DelayedPDActuator):
    ...
    def __init__(self, cfg: "MeasuredFrictionDelayedPDActuatorCfg", *args, **kwargs):
        ...
        print(
            ...
            f"effort_limit_nm={cfg.effort_limit} "
            f"velocity_limit_rad_s={cfg.velocity_limit_sim} "
            f"stiffness_nm_per_rad={cfg.stiffness} "
            f"damping_nm_s_per_rad={cfg.damping} "
            f"armature={cfg.armature} "
            f"delay_physics_steps={cfg.min_delay}..{cfg.max_delay}",
            flush=True,
        )
