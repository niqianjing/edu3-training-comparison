...
class MotionLoader:
    def __init__(self, motion_file_path, simulator_joint_names, device = "cuda:0", add_static_frame: bool = False):
        self.device = device
        loaded_data = joblib.load(motion_file_path)
        self.root_trans = torch.from_numpy(loaded_data["default"]["root_trans_offset"]).to(self.device)
        self.root_rot = torch.from_numpy(loaded_data["default"]["root_rot"]).to(self.device)
        self.root_rot[:] = self.root_rot[:, [3, 0, 1, 2]]# xyzw->wxyz
        # self.pose_aa = torch.from_numpy(loaded_data["default"]["pose_aa"]).to(self.device)
        self.capture_points = torch.from_numpy(loaded_data["default"]["capture_points"]).to(self.device)
        # self.capture_points[..., 2] -= 0.06 # TODO: 降低了一点capture point的高度
        self.dof_pos = torch.from_numpy(loaded_data["default"]["dof"]).to(self.device)
        self.fps = loaded_data["default"]["fps"]
        ...
        self.capture_points_link_names = loaded_data["marker_link_names"]
        self.mjcf_joint_names = loaded_data["mjcf_joint_names"]
    ...
    def _trans_dof_pos(self, simulator_joint_names):
        ...
        for joint_name in simulator_joint_names:
            joint_index.append(self.mjcf_joint_names.index(joint_name))
