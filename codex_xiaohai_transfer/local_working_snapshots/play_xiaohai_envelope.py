...
def main(args):
    ...
    # Xiaohai original controller has no EDU3 continuous torque table.
    continuous = None
    if hasattr(env, "edu3_continuous"):
    ...
    for _ in range(steps):
        ...
        motor_torque = getattr(env, "edu3_motor_torques", env.torques)
        friction_torque = getattr(env, "edu3_friction_torques", torch.zeros_like(env.torques))
        net_torque = getattr(env, "edu3_net_torques", env.torques)
        logs["motor"].append(motor_torque[0].detach().cpu().numpy().copy())
        logs["friction"].append(friction_torque[0].detach().cpu().numpy().copy())
        logs["net"].append(net_torque[0].detach().cpu().numpy().copy())
    ...
    for i, name in enumerate(names):
        joints[name] = {
            ...
            "continuous_ratio": (None if continuous is None else float(np.sqrt(np.mean(motor[:, i] ** 2)) / continuous[i])),
